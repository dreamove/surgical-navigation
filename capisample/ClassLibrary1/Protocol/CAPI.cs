...
namespace NDI.CapiSample.Protocol
{
    ...
    public abstract class Capi
    {
        ...
        //不用于初始化的命令需要快速响应
        public Command Send1(string command, bool expectResponse = true)
        {
            var cmd = new Command(command);
            var task = Send(cmd, expectResponse);
            try
       ...
       // public List<Tool> SendBX(int options = (int)BxReplyOptionFlags.ToolMarkerData)
        {
            ...
            //var command = Send1($"BX {options:X4}");
            var command = Send1($"BX {options:X4}");
            if (command.HasValidResponse)
            {
